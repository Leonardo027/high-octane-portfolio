...
const FILE_SYSTEM: any = {
  root: {
    home: {
      visitor: {
        "welcome.txt":
          "Bem-vindo ao LC_TERMINAL v1.0. Use 'help' para ver os comandos.",
        "about.md":
          "Sou Leonardo Cantelle. Desenvolvedor Full-Stack e entusiasta de Hardware.",
        projects: {
          "jarvis_log.txt":
            "LOG #001: Reconhecimento de voz ativado. Integração com ESP32 pendente.",
        ...
        secrets: {
          "password_hint.txt":
            "A senha do sudo é o nome da melhor escola técnica...",
        },
      },
    },
    etc: {
...
export default function Terminal() {
  ...
  const [history, setHistory] = useState<HistoryItem[]>([
    ...
    {
      type: "success",
      content: "Welcome to LC_TERMINAL. Type 'help' to start.",
    },
  ]);
  ...
  const handleCommand = async (cmd: string) => {
    ...
    const newHistory = [
      ...history,
      { type: "input" as CommandType, content: cmd, path: getDisplayPath() },
    ];
    ...
    if (isSudoMode) {
      if (cmd === "senai" || cmd === "SENAI" || cmd === "admin") {
        newHistory.push({
          type: "success",
          content: "ACCESS GRANTED. Root privileges active (simulated).",
        });
        newHistory.push({
          type: "output",
          content:
            "Secret Project Unlocked: https://github.com/Leonardo027/Hackpad_Hack_Club",
        });
      } else {
        newHistory.push({
          type: "error",
          content: "Authentication failed. Incident reported.",
        });
      }
    ...
    switch (command) {
          ...
          content: (
            <div className="grid grid-cols-1 gap-1 text-sm">
              ...
              <div className="grid grid-cols-[100px_1fr] gap-2">
                <span className="text-neon">ls</span>{" "}
                <span>List directory content</span>
                <span className="text-neon">cd [dir]</span>{" "}
                <span>Change directory</span>
                <span className="text-neon">cat [file]</span>{" "}
                <span>Read file content</span>
                <span className="text-neon">clear</span>{" "}
                <span>Clear terminal screen</span>
                <span className="text-neon">weather</span>{" "}
                <span>Check real-time weather</span>
                <span className="text-neon">sudo</span>{" "}
                <span>Execute as root</span>
        ...
        if (dirContent && typeof dirContent === "object") {
          const items = Object.keys(dirContent).map((key) => {
            ...
            return (
              <span
                key={key}
                className={`mr-4 ${isDir ? "text-blue-400 font-bold" : "text-white"}`}
              >
                {key}
                {isDir ? "/" : ""}
              </span>
          ...
          newHistory.push({
            type: "output",
            content: <div className="flex flex-wrap">{items}</div>,
          });
        } else {
          newHistory.push({
            type: "error",
            content: "Error: Cannot list content of a file.",
          });
        }
        break;
      ...
      case "cd":
        if (!target) {
          ...
          if (
            currentDir &&
            currentDir[target] &&
            typeof currentDir[target] === "object"
          ) {
      ...
      case "sudo":
        newHistory.push({
          type: "output",
          content: "Enter password for root:",
        });
        setIsSudoMode(true);
        setHistory(newHistory);
      ...
      case "weather":
        newHistory.push({
          type: "output",
          content: "Contacting meteorological satellites...",
        });
        ...
        try {
          ...
          setHistory((prev) => [
            ...prev,
            { type: "error", content: "Connection failed: Satellite offline." },
          ]);
        }
        return;
        ...
        >
          {history.map((item, index) => (
            <div key={index} className="mb-1 break-words leading-relaxed">
              ...
              {item.type === "success" && (
                <div className="text-green-400 ml-4">{item.content}</div>
              )}
            </div>
          ))}
